...
// ⚙️ BẢO MẬT KẾT QUẢ
// Không ghi RESULTS_SECRET trực tiếp trong mã nguồn.
// Vào Project Settings → Script Properties và tạo RESULTS_SECRET.
// Giá trị này phải giống hoàn toàn với RESULTS_SECRET trên Vercel.
var RESULTS_SHEET_NAME = 'Kết quả';

// ⚙️ TÊN THƯ MỤC trên Google Drive để lưu ảnh bài làm của học sinh.
var IMAGE_FOLDER_NAME = 'AnhBaiLam_HocSinh';

// ===== TIỆN ÍCH API KẾT QUẢ =====
...
function getResultsSecret_() {
  return String(
    PropertiesService.getScriptProperties().getProperty('RESULTS_SECRET') || ''
  );
}

function isResultsAuthorized_(providedSecret) {
  var expectedSecret = getResultsSecret_();
...
function doPost(e) {
  try {
    ...
    // Mọi request ghi/xóa đều phải có khóa bí mật do Vercel gửi.
    if (!isResultsAuthorized_(data.secret)) {
      return jsonOutput_({
        status: 'error',
    ...
    // Lệnh xóa phải được xử lý trước luồng lưu bài và gửi email.
    if (data.action === 'deleteResults') {
      return deleteResults_(data);
    }
...
function doGet(e) {
  ...
  if (String(params.action || '') !== 'results') {
    return ContentService.createTextOutput('Google Apps Script đang hoạt động! ✅');
  }
  if (!isResultsAuthorized_(params.secret)) {
    return jsonOutput_({ status: 'error', message: 'Không có quyền truy cập.' });
  }
  ...
  try {
    ...
    // Mới nhất lên đầu nhưng rowNumber vẫn là số dòng thật trong Google Sheet.
    rows.reverse();
...
// ===== XÓA KẾT QUẢ =====
// Chỉ do Vercel gọi qua POST với action = deleteResults.
function deleteResults_(data) {
  ...
  try {
    ...
    var lastRow = sheet.getLastRow();
    if (lastRow < 2) {
      return jsonOutput_({ status: 'success', deleted: 0 });
    }

    if (data.scope === 'all') {
      var total = lastRow - 1;
      sheet.deleteRows(2, total);
      return jsonOutput_({ status: 'success', deleted: total });
    }

    var seen = {};
    var rowNumbers = Array.isArray(data.rowNumbers) ? data.rowNumbers : [];
    var rows = rowNumbers
    ...
    // Xóa từ dưới lên để số dòng phía trên không bị thay đổi giữa chừng.
    rows.forEach(function(rowNumber) {
      sheet.deleteRow(rowNumber);
    });

    return jsonOutput_({ status: 'success', deleted: rows.length });
  } catch (err) {
    return jsonOutput_({ status: 'error', message: err.toString() });
  } finally {
    lock.releaseLock();
